...
export default function PhotoUploader({ addPhotos, onChange }) {
  ...
  function removePhotos(ev, li) {
    ev.preventDefault();
    onChange([...addPhotos.filter((photo) => photo !== li)]);
  }

  function selectAsMainPhoto(ev, li) {
    ev.preventDefault();
    onChange([li, ...addPhotos.filter((photo) => photo !== li)]);
  }
  ...
  return (
    <>
      ...
      <div className="mt-2 grid gap-1 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        ...
        <label className="h-32 cursor-pointer flex  items-center gap-1 justify-center border gap-1 bg-transparent rounded-2xl p-8 text-xl text-gray-600">
          <input
            type="file"
            multiple
            className="hidden"
